...
const ShaderMaterial = ({ source, uniforms, maxFps = 60 }) => {
  ...
  //         ? uniform.value.map((v) => new THREE.Vector3(...v))
  //         : uniform.value,
  //       type: uniform.type,
  //     };
  //   });
  ...
  const getUniforms = useMemo(() => {
    ...
    Object.entries(uniforms).forEach(([key, uniform]) => {
      preparedUniforms[key] = {
        value: Array.isArray(uniform.value)
          ? uniform.value.map((v) => {
              // Ensure v is an array of length 3
              if (Array.isArray(v) && v.length === 3) {
                return new THREE.Vector3(...v);
              } else {
                console.warn(`Expected array of length 3 for ${key}, got:`, v);
                return new THREE.Vector3(0, 0, 0); // Fallback to default
              }
            })
          : uniform.value,
        type: uniform.type,
